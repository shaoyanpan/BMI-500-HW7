...
df = pd.read_csv('C:/PAN/BMI 500/HW9/first_ten.csv')
df = df.sample(frac=1).reset_index(drop=True)
columns = df.columns
labels = df['aki_stage'].values
onehot_labels = pd.get_dummies(labels).values

#Interpolate the missing values and drop the aki stage (label) and string features
df.drop(['creat','pao2_aado2_score','aki_stage','specimen','albumin_score','antibiotic_name'],axis=1,inplace=True)
df = df.astype(float)
df = df.interpolate(method='linear', axis=0).ffill().bfill()
mat = df.values
# oversample = SMOTE()
# mat, labels = oversample.fit_resample(X_train, labels)
